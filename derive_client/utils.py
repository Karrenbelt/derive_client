...
def wait_for_tx_receipt(
    w3: Web3, tx_hash: str, timeout=120, poll_interval=1
) -> AttributeDict:
...
def exp_backoff_retry(
    func=None, *, attempts=3, initial_delay=1, exceptions=(Exception,)
):
    if func is None:
        return lambda f: exp_backoff_retry(
            f, attempts=attempts, initial_delay=initial_delay, exceptions=exceptions
        )
...
EIP1967_SLOT = (
    int.from_bytes(Web3.keccak(text="eip1967.proxy.implementation")[:32], "big") - 1
).to_bytes(32, "big")
...
def get_impl_address(w3: Web3, address: str) -> str | None:
    ...
    if int(impl_address, 16) == 0:  # not a proxy
        return
    return impl_address
    ...
    for chain_id, addresses in chain_addresses.items():
        ...
        while addresses:
            ...
            if impl_address := get_impl_address(w3=w3, address=address):
                logger.info(
                    f"EIP1967 Proxy implementation found: {address} -> {impl_address}"
                )
